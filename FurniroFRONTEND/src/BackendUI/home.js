...
function Home() {
  ...
  const fetchData = async () => {
    try {
      ...
      const categoryResponse = await axios.get('http://localhost:5000/api/categorypage/getcategory');
      setCategories(categoryResponse.data.data.length); // Adjust according to your API response structure

      // Fetching products data
      const productResponse = await axios.get('http://localhost:5000/api/productpage/getproductvariety');
      ...
      const usersResponse = await axios.get('http://localhost:5000/api/user/getuser');
      setUsers(usersResponse.data.data.length); 
      
      // Adjust according to your API response structure

      // Fetching orders data - add your API endpoint for orders
      const ordersResponse = await axios.get('http://localhost:5000/api/orderpage/getorderitems');
