  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/productpage/products",
        formData
      );
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(
        `http://localhost:5000/api/productpage/productsdelete/${id}`
      );
      if (response.data.success) {
    ...
    const fetchData = async () => {
      try {
        const catg = await axios.get(
          "http://localhost:5000/api/categorypage/getcategory"
        );

        setCategoryData(catg.data.data);

        const prod = await axios.get(
          "http://localhost:5000/api/productpage/getproducts"
        );
  ...
  return (
    <div>
      <div className="grid-container">
        ...
        <div className="pro-card-head" style={{ margin: "0", padding: "0" }}>
